...
- [x] **Modificaciones de inventario offline (otro gap encontrado, tampoco estaba en el alcance original):** bloqueadas — todas las escrituras de `MenuContext` (`agregarProducto`, `editarProducto` y lo que envuelve — `toggleActivo`/`ajustarStock`/`generarCodigoBarras`—, `borrarProducto`, `toggleFavorito`, `togglePublicado`, `registrarMovimientoStock`, `actualizarPreciosMasivo`, categorías) tiran error explícito (`requiereConexion()`) si `!online`. Sin deshabilitar botones puntuales en cada pantalla (son muchas: Inventario, sus ~6 modales) — se confía en el `BannerOffline` global + el mensaje de error inline que cada modal ya muestra.
  - **Considerado para el futuro, no encarado:** encolar `registrarMovimientoStock` (ingreso/ajuste/merma/devolución) con el mismo patrón que las ventas — a diferencia del resto de las escrituras de inventario, ya es una operación de **delta** (no un reemplazo), así que reusaría casi toda la infraestructura de `colaVentas`/`reconciliacion.ts`. El resto (`editarProducto` y compañía, que SÍ son reemplazos) es un diseño bastante más grande, se evalúa por separado si hace falta.
