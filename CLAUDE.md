...
- **`authorized` ≠ paid — a real gotcha we hit in production:** a preapproval's `status: 'authorized'` only means the payer approved the recurring mandate; the actual charge happens separately (~1h later per MP docs) and can fail, and MP doesn't pause the subscription on a single failed charge (it retries up to 4×/installment over 10 days, only auto-cancelling after 3 fully-rejected installments). So `webhook-mp` does NOT set `suscripcion_estado = 'activa'` on `authorized` — it only reacts to `paused`/`cancelled` there. Activation instead comes from the separate `subscription_authorized_payment` webhook topic (subscribed via `save_webhook` alongside `subscription_preapproval`): on that event, `webhook-mp` fetches `GET /authorized_payments/{id}` and only marks `activa` when the nested `payment.status === 'approved'` — the outer `status` field on that resource can read `"processed"` both when a charge succeeded AND when it was rejected after exhausting retries, so the real signal is the nested `payment.status`, not the outer one.
...
- **In production**: `MP_ACCESS_TOKEN` is the real `APP_USR-` token, the webhook is configured in the production tab of the MP panel (topics: `subscription_preapproval`, `subscription_preapproval_plan`, `subscription_authorized_payment`), and real subscriptions are confirmed charging.
